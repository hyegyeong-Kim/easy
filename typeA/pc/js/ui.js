...
(function($, core, ui, undefined){
	...
	ui.PARALLAX_ANIMATION = function(){
		...
		var events = this.events = {
            ...
            if(data[i].function !== undefined){
              eval(data[i].function);
            }
						++complete;
					}
  ...
  ui.slideSec2 = function(){
		...
		this.events = {
			_init: function(){
				$$.nextBtn.on("click", $.proxy(this._nextClick, this));
			},
			/*
			_detect: function(dir){
				var dir = (dir !== -1) ? 1 : -1;
				var index = 0;
  ...
  ui.slideSec4 = function(){
		...
		this.events = {
			...
			_nextClick: function(e){
				...
				sec4_swiper3.slideNext();
				return false;
			}
		}
	}
	ui.slideSec5 = function(){
		var $$ = core.Selector(arguments[0], {
			nextBtn : ".sec5_next",
		});
		...
		this.events = {
			...
			_nextClick: function(e){
				swiper3.slideNext();
				swiper4.slideNext();
				TweenMax.to(".txt_swiper img", 0.5, {opacity:0,
					onComplete: function(){
						TweenMax.to(".txt_swiper img", 1, {opacity:1, delay:0.5});
					}
				});
				return false;
			}
	...
	ui.popSlide = function(){
		...
		this.events = {
			_init: function(){
				$$.nextBtn.on("click", $.proxy(this._nextClick, this));
				$$.prevBtn.on("click", $.proxy(this._prevClick, this));
				$$.closeBtn.on("click", $.proxy(this._closeClick, this));
			},
			_nextClick: function(e){
				swiper3.slideNext();
				swiper4.slideNext();
			...
			_closeClick: function(e){
				$('.popup_wrap').hide();
				$('.popup_wrap').attr('style','');
				$('.pop_left').css('opacity','0');
				TweenMax.set( $('.sec2 .swiper-slide-active img') , {opacity: 1});
				return false;
			},
		}
	}
